...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    checkMultipleUploads: function(){
        if($$('.form-upload-multiple').length > 0){
            ...
            script.src = "https://static-interlogyllc.netdna-ssl.com/file-uploader/fileuploader.js?v6";
...
{       // Event simulator
    ...
    * @param {type} text
    ...
    /**
     * Checks whether a payment item has been selected
     * @returns {Boolean}
     */
    
    isPaymentSelected: function () {
        // if there is no payment field, 
    ...
    handlePaypalButtons: function () {
        ...
        var paymentFieldId = $$('input[name="simple_fpc"]')[0].value;
        // change paypal button image for PayPal demo forms
        var formID = $$('input[name="formID"]')[0].value;
        $$('.form-submit-button').each(function(btn){
            if (formID === "42050832341948") {
                btn.siblings()[0].setAttribute('src', 'https://www.paypalobjects.com/webstatic/en_US/i/buttons/buy-logo-medium.png');
            }
            if (formID === "42034708572958" ) {
                btn.siblings()[0].setAttribute('src', 'https://www.paypalobjects.com/webstatic/en_US/i/buttons/checkout-logo-medium.png');
            }
        });
    ...
    handlePaypalStandard: function () {
        ...
        function handlePostMessage(e) {
            ...
            try {
                ...
                console.log(err);
                window.alert('An error has occurred. Please try again or contact the form owner.');
            }
        }
        Event.observe(thisForm, 'submit', function (e) {
            if (!JotForm.validateAll()) {
                return;
            } else if (!JotForm.isPaymentSelected()) {
                thisForm.submit();
            } else {
                Event.stop(e);
                $$('.for-submit-button').each(function (btn) {
                    btn.disable();
                ...
                thisForm.submit();
                setTimeout(function(){
                    // change back target after submitting
                    thisForm.writeAttribute('target', '');
                    // remove submit
