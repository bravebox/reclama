...
function ($) {
	...
	// Init
		var initialize = function(a, b) {
            ...
            }), d.parent().css("overflow", "hidden"), d.css(i > h ? {
                height: "auto",
                width: e
            } : {
                height: c,
...
function ($, Detect, VideoScale) {
	

	// video player
		var video_player = document.getElementById("case_video");
	...
	// Elements
		var el = {
				body : 'body',
			// video
				video_slide : '.case_video',
				video_play_button : '#play-btn'
		}
	...
	// Init
		var initialize = function() {
			...
			// browser data
				var browser_data = Detect.initialize();
		
			// masonry if older version
				if(browser_data.browser === 'Explorer' && browser_data.version <= '9') {
					// action
				}

			// video player 
				$(document).on('click', "#play-btn", function(e) {
					if(video_enabled === true) { // video_enabled -> single-case.php
						toggleVideo();
					}
					e.preventDefault();
				});
			...
			// video : scale
				if(!video_enabled || video_enabled === true) { // video_enabled -> single-case.php
					setTimeout(function() {
						VideoScale.initialize(1920, 1080);						
					}, 1000);
				}

			// open slideshow
				$(document).on('click', ".case-slide-img", function(e) {

					var index = $(this).parent().parent().index();
						$("#case-slideshow-single").cycle('goto', index);
...
function ($, Detect, Sticky, Dropdown, Case) {
	...
	// Init
		var initialize = function() {
			...
			// menu animation
				$(el.loader).delay(1500).fadeOut('slow', function() {
					if(browser_data.browser === 'Explorer' && browser_data.version <= '9') {
						$(el.menu_container).find('.front').hide();
						$(el.menu_container).find('.back').animate({opacity: 1}, 300);
					} else {
						$(el.menu_container).find('.front').addClass('add_flip_out');
						$(el.menu_container).find('.back').addClass('add_flip_in');
					}
				});

			// menu
				$(document).on("click", el.open_overlay, function(e) {
					e.preventDefault();
						openMenu();
				});

				$(document).on("click", el.close_overlay, function(e) {
					e.preventDefault();
						closeMenu();
				});

			// sticky
				$(el.sticky_class).sticky({topSpacing:0});

			// sticky links
				$(document).on('click', el.tab_class, function(e) {
					e.preventDefault();
			...
			// add .goto to a link and an #id as href to animate to that section
				$(document).on('click', el.go_to, function(e) {
	...
	// Menu Functions
		...
		var closeMenu = function() {
			$('#case-slideshow').hide(); // single case
			
			$(el.menu).removeClass('active').fadeOut('fast');
			$(el.body).removeClass('no-scroll');
		};

	// Tab Functions
		var tabLink = function(href, index) {
			// top position
				var top = $(el.info_case).offset().top - 110;

			// active class
				$(el.tab_class).removeClass('active');
				$(el.tab_class+':eq('+index+')').addClass('active');

			// exception and animate
				if(href === "#afspraak") {
					$('body, html').animate({ scrollTop: $(document).height() }, 1200);
				} else {
					$('body, html').animate({scrollTop: top}, 800, function() {
						$(el.info_case).cycle('goto', index);
					});
				}
		};
...
function ($, VideoScale) {
	...
	// Resize
		...
		var scaleFs = function() {
			var window_height = $(window).height();
				$(el.fs).css({height: window_height});
				$(el.fs_case).css({height: window_height - 120});
		}
...
function ($, Cycle2_lib, Cycle2_carousel, Cycle2_fade) {
	...
	{	
		// vars
			var el = {
				// trigger
					trigger: '.cycle_carousel',
				// containers
					home_cases: '#cases_home',
		...
		// Init only cases homepage
			$('.owl-prev').find('.case-item:last-child').show();
			$('.owl-next').find('.case-item:eq(1)').show();

		// Home : Cases inspiration slideshow
			$(el.home_cases).on('cycle-after', function() {
				setCaseInfo( $(el.home_cases).find('div.cycle-slide-active').index() );
			});

		// Home : Info
			$(el.info_case).on('cycle-bootstrap', function(e, opts, API) {
			    API.log = function() {
					$('a.owl-tab').removeClass('active');
					$('a.owl-tab:eq('+opts.currSlide+')').addClass('active');
			    };
			});

		// Single : Single case
			/*
			el.single_case.on('cycle-initialized cycle-before', function() {
			    el.progress.stop(true).css( 'width', 0 );
			});
			
			el.single_case.on('cycle-initialized cycle-after', function( e, opts ) {
			    if ( ! el.single_case.is('.cycle-paused') ) {
			        el.progress.animate({ width: '100%' }, opts.timeout, 'linear' );
			    }
			});
			
			el.single_case.on('cycle-paused', function() {
			   el.progress.stop(); 
			});
			
			el.single_case.on('cycle-resumed', function( timeoutRemaining ) {
				el.progress.animate({width: '100%' }, timeoutRemaining, 'linear');
			});
				
			$(el.single_case).on('cycle-bootstrap', function(API) {
			    API.log = function() {
					el.progress.stop(true);
			    };
			});
			*/
	};

	var setCaseInfo = function(c) {
		var ch;
			clearInterval(ch);
			...
			ch = setInterval(function() {
				...
				$('.case-item').hide();
				$('.owl-prev').find('.case-item:eq('+prev_index+')').fadeIn('fast');
				$('.owl-next').find('.case-item:eq('+next_index+')').fadeIn('fast');
...
function ($, Detect) {
	...
	// Elements
		var el = {
			...
			// load containers
				...
				cases_container: '#cases_container',
				cases_pagination: '#cases_pagination',
				cases_notification: '#cases_notification',
			// urls
				search_url: '/cs/',
			// triggers
				search: '#cases_search',
				taxonomy: '.taxonomy_group li a',
				dates: '.date_group li a',
				pagination: 'a.pagi',
				reset: 'a.cases_reset',
			// notifications
	...
	// Init
		var initialize = function() {
			...
			// click : pagination
				$(document).on('click', el.pagination, function(e) {
					if( el.busy === false ) {
							el.busy = true;
						// active class
							$(el.pagination).removeClass('active');
							$(this).addClass('active');
						// load function
							getFromPagination( $(this).attr('href') );
					}
					e.preventDefault();
				});

			// click : taxonomy
				$(document).on('click', el.taxonomy, function(e) {
					if( el.busy === false ) {
						getTaxonomy( $(this).attr('href') );
					}
					e.preventDefault();
				});

			// click : dates
				$(document).on('click', el.dates, function(e) {
					if( el.busy === false ) {
						getSearchResults( $(this).attr('data-search'), $(this).attr('data-value') );
					}
					e.preventDefault();
				});

			// click : reset	
				$(document).on('click', el.reset, function(e) {
					if( el.busy === false ) {
						$(el.s).val(''); // clear search
						getTaxonomy( $(this).attr('href') );
					}
					e.preventDefault();
				});			
			...
			// submit : search
				$(el.search).on('submit', function(e) {
					if( el.busy === false ) {
						getSearchResults( 'search', $(el.s).val() );
					}
					e.preventDefault();
				});

			// type : accordion toggle
				$(document).on('click', 'a.accordion-parent', function(e) {
					$("#accordion_grp_1").find('ul').hide();
	...
	// Functions
		var startSearch = function() {
			el.busy = true;
			$(cases_loader).show();		
			$(el.cases_container).animate({opacity: 0.2}, 200);
		};
		
		var endSearch = function() {
			el.busy = false;
			$(el.cases_loader).fadeOut('slow');
			$(el.empty).empty();
		};

	// Pagination
		var getFromPagination = function(url) {
			// start
				startSearch();
			// scroll to top and load URL
				$("body, html").animate({ scrollTop: 0 }, "slow", function() {
					// load
						$(el.empty).load( url + ' ' + el.cases_container, {}, function() {
							$(el.cases_container).empty().hide();
							$(el.cases_container).replaceWith( $(el.empty).html() ).hide().fadeIn('slow', function() {
								endSearch();
							});
						});
				});							
				...
				$("body, html").animate({ scrollTop: 0 }, "slow", function() {
					// load
						$.get(url, {}, function(data) {
							...
							// broken down
							    var $header = $response.find(el.cases_header);
							    var $cases = $response.find(el.cases_container);
							    var $pagination = $response.find(el.cases_pagination);
	
							// replace content
							    $(el.cases_header).replaceWith($header);
							    $(el.cases_container).replaceWith($cases);
							    $(el.cases_pagination).replaceWith($pagination);
	...
	// Search function
		var getSearchResults = function( search_type, search_value ) {
			// start
				startSearch();
					...
					$.get(el.search_url, {
						...
						// check / replace content
							if( $cases.html().length > 0 ) {
								...
								setNotification(el.cases, el.no_result);
							}
	...
	// Search notifications
		var setNotification = function(container, message) {
			if(container) {
				$(container).prepend("<div id='cases_notification'>"+message+"</div>");
				var ch = setInterval(function() {
					$(el.cases_notification).fadeOut('slow', function() {
						$(this).remove(); 
					});
					clearInterval(ch);
					return false;					
...
function($, Modernizr, Bootstrap, General, Resize, Social, Detect, Cycle, Cases, Case) {
...
require.config({
	paths: {
		// core
			'wijze': 'core/wijze', // module loader
			'general': 'core/general', // some general clicks
			'detect': 'core/detect', // detect browser, animates loader
			'resize': 'core/resize', // resize function 
			'social': 'core/social', // social share links
			'cycle2': 'core/cycle2', // home, single case
			'cases': 'core/cases', // cases (search, filter clicks)
			'case': 'core/case', // single case (video, slideshow)
			'video_scale': 'core/video_scale', // function to scale video fullscreen
